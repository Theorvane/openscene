...
/**
 * Image generation adapters. Each provider is reached over its own documented
 * HTTP surface; the shared shape here is only the input and the returned bytes.
 *
 * fetch is injectable for the same reason the video and speech adapters make it
 * injectable: these paths are otherwise only exercised with live credentials.
 */
...
export type ImageSynthesisInput = {
...
function decodeBase64Image(base64: string, mimeType: string, providerJobId: string): GeneratedImage {
  const bytes = Buffer.from(base64, 'base64');
  if (bytes.length === 0) {
    throw new Error('The provider returned an empty image payload.');
  }
  return { bytes, mimeType, providerJobId };
}

/** Some providers return a URL instead of inline bytes; fetch it rather than failing. */
async function fetchImageUrl(url: string, providerLabel: string, fetchImpl: typeof fetch): Promise<GeneratedImage> {
  const response = await fetchImpl(url);
  await expectOk(response, providerLabel);
...
export async function generateOpenAiImage(input: ImageSynthesisInput): Promise<GeneratedImage> {
  ...
  if (first?.b64_json !== undefined) {
    return decodeBase64Image(first.b64_json, 'image/png', `openai-image-${parsed.created ?? 'result'}`);
  }
 ...
 * response handling below is deliberately the same union of inline bytes or a
 * URL rather than a second parser.
 */
export async function generateBytePlusImage(input: ImageSynthesisInput): Promise<GeneratedImage> {
  ...
  const first = parsed.data?.[0];
  if (first?.b64_json !== undefined) {
    return decodeBase64Image(first.b64_json, 'image/png', `seedream-${input.modelId}`);
  }
  if (first?.url !== undefined) {
