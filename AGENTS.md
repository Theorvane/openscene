...
9. Store listings are filled in from `store/`, not written into the consoles from memory. `store/app-store/whats-new.txt` and `store/google-play/release-notes.txt` are the copy; paste them. The App Store text may not name another platform — App Review refuses it under Guideline 2.3.10, and `tests/storeReleaseNotes.test.ts` refuses it first.
