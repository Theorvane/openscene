...
describe('app workspaces', () => {
  it('returns a stable four-workspace model without Prompt Studio', () => {
    // Given / When
    ...
    expect(workspaces.map((workspace) => workspace.statusLabel)).toEqual(['Local NLE', 'Local & API', 'Local & API', 'Preferences']);
  });

  it('moves focus across the four workspaces with ArrowUp, ArrowDown, Home, and End wrap', () => {
    // Given / When / Then
