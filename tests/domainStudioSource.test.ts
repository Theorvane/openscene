...
const PROMPT_SOURCE_URL = new URL('../src/renderer/src/PromptStudioWorkspace.tsx', import.meta.url);
...
describe('domain generation studio wiring', () => {
  it('uses the voice and video domain selectors instead of the global LLM selector', async () => {
    const [video, prompt, voice] = await Promise.all([
      readFile(VIDEO_SOURCE_URL, 'utf8'),
      readFile(PROMPT_SOURCE_URL, 'utf8'),
