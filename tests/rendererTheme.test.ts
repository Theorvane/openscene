...
describe('renderer theme contract', () => {
  ...
  it('Given the persistent agent chat rail, When renderer CSS is checked, Then only the activity log scrolls inside a viewport-bounded panel', () => {
    ...
    // The side panel owns the frame; the chat body adds the tab-bar row.
    expect(rendererStyles).toMatch(/\.side-panel\s*\{[\s\S]*?display:\s*grid;[\s\S]*?grid-template-rows:\s*auto minmax\(0, 1fr\);[\s\S]*?overflow:\s*hidden;/);
    expect(rendererStyles).toMatch(/\.agent-chat-panel\s*\{[\s\S]*?display:\s*grid;[\s\S]*?grid-template-rows:\s*auto auto auto minmax\(0, 1fr\) auto;[\s\S]*?max-height:\s*100%;[\s\S]*?overflow:\s*hidden;/);
